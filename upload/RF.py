...
for i in range(len(specieslist)):
    ...
    for n in range(1,6):
        ...
        rf_model = joblib.load(model_path+name+'_'+str(n)+'_rf.pkl')
        model_importance['feature'] = rf_model.feature_names_in_
        model_importance['imp'+str(n)] = rf_model.feature_importances_
        if n == 1:
    ...
    importance.to_csv(imp_path+name+'_rf.csv',index=False)
...
for i in range(len(specieslist)):    
    ...
    for k in range(len(feature_list)):
        ...
        for n in range(1,6):
            eff_0 = pd.DataFrame()
            data = pd.read_csv(training_data_path+name+'_'+str(n)+'_train.csv')
            model = joblib.load(model_path+name+'_'+str(n)+'_rf.pkl')
            climate = data.drop(feature_drop,axis=1,inplace = False)
            p = ALE(X=climate, model=model, feature=[feature],grid_size=1000,include_CI=True,C=0.95)
            eff_0[feature+'_x'+str(n)] = p.index
            eff_0[feature+'_y'+str(n)] = p['eff'].values
            eff = pd.concat([eff,eff_0],axis=1,join='outer')    
            pass
        eff.to_csv(interpret_path+name+'_'+feature+'_rf.csv',index=False)
